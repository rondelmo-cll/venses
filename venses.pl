...
#%C0%C1%C2%C3%C4%C5%C6%C7%C8%C9%CA%CB%CC%CD%CE%CF%D0%D1%D2%D3%D4%D5%D6%D7%D8%D9%DA%DB%DC%DD%DE%DF%E0%E1%E2%E3%E4%E5%E6%E7%E8%E9%EA%EB%EC%ED%EE%EF%F0%F1%F2%F3%F4%F5%F6%F7%F8%F9%FA%FB%FC%FD%FE%FF
    ...
    # mengwong: lol wat? 20160517 ... why not extract this using a proper CGI library?
...
OUTRO
			...
			# lolwut, is this really necessary?
			# $var{"text"}=~s/([^ ])/\\\1/sg;
			# $var{"hypo"}=~s/([^ ])/\\\1/g;
			
			$ENV{VENSES_TEXT} = encode('UTF-8',$var{'text'});
			$ENV{VENSES_HYPO} = encode('UTF-8',$var{'hypo'});
			$new="./myprog \$VENSES_TEXT \$VENSES_HYPO";

			$|++;
			# we really should give the user a spinner, here
			print("Content-type: text/html\n\n");
			print("<html><head></head><body><p>running ...</p>\n");

			
			print STDERR "export VENSES_TEXT=\"$ENV{VENSES_TEXT}\"\n";
			print STDERR "export VENSES_HYPO=\"$ENV{VENSES_HYPO}\"\n";
			print STDERR "venses calling myprog: orig string = $new\n";
			use HTML::Entities;
			$output=qx($new);
			print STDERR "venses calling myprog: output = $output\n";
			my @output = grep { ! /^DEBUG/ } split(/\n/,$output);
			$output = encode_entities (join "\n",@output);
